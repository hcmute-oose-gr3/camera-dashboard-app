<script lang="ts">
	...
	export let profiles: (Profile & { _id: string })[];
	export let checked: boolean[];

	let deletePendings: boolean[] = [];
	$: ({ table: text } = $LL.dashboard.profiles);
	...
	async function deleteThis(index: number) {
		...
		const json = await fetch(`/api/v1/dashboard/${$page.params.id}/profiles/${profile._id}`, {
			method: 'delete',
		})
			.then((v) => v.json())
			.finally(() => {
	...
	async function deleteSome() {
		const deleting = profiles.filter((_, i) => checked[i]).map((c) => c._id);
		deletePendings = checked.map((v) => v);
		console.log(deletePendings);
		const json = await fetch(`/api/v1/dashboard/${$page.params.id}/profiles`, {
			method: 'delete',
...
<table class="table-auto text-left w-full transition-all">
	...
	<tbody>
		{#each profiles as profile, i (profile._id)}
			<tr
			...
			>
				...
				<td class="block lg:table-cell px-4 py-6"
					...
					>{parseInt(profile._id.substring(profile._id.length - 6), 16)}</td
				>
				<td class="block lg:table-cell px-4 py-6"
					><span class="lg:hidden inline-block w-36 text-fill-700 font-bold"
				...
				<td class="lg:hidden pr-4 w-0">
					<div class="flex flex-col gap-y-4 items-center justify-between">
						<a href="/dashboard/{$page.params.id}/profiles/{profile._id}/edit">
				...
				<td class="hidden lg:table-cell px-4 py-6">
					<div class="flex gap-x-4 items-center">
						<a
							class="flex"
							href="/dashboard/{$page.params.id}/profiles/{profile._id}/edit"
						>
							<SecondaryButton class="text-yellow-600 border-yellow-600 p-0.5">
								<Icon name="Pencil" class="w-7 h-7" />
							</SecondaryButton>
